...
zokou({ nomCom: "hacker",
    ...
    if (!arg || arg == "") {
        repondre("*__Exemple : * " + prefixe + "hacker bustarzone");
        return;
    }
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("* processing...*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t Logo by ʙᴜsᴛᴀʀᴢᴏɴᴇ ᴍᴅ*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by ʙᴜsᴛᴀʀᴢᴏɴᴇ ᴍᴅ v5*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        if (!arg || arg == '') {
            repondre("*_Exemple : * " + prefixe + "naruto bustarzone md");
            return;
        }
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by ʙᴜsᴛᴀʀᴢᴏɴᴇ ᴍᴅ *" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "didong", categorie: "Logo", reaction: "📱" }, async (dest, zk, commandeOptions) => {
    let { arg, repondre, prefixe, ms } = commandeOptions;
   try {
     var titre = "\t* logo by ʙᴜsᴛᴀʀᴢᴏɴᴇ ᴍᴅ v5*";
      var lien = "https://ephoto360.com/tao-anh-che-vui-tu-choi-cuoc-goi-voi-ten-cua-ban-930.html";
        if (!arg || arg == "") {
           repondre(`*exemple :* ${prefixe}didong Fredie`)
            return ;
  ...
  {nomCom:"boom",categorie:
    ...
    let {ms,repondre,prefixe,arg}=commandeOptions;
        ...
        if(!arg||arg=="")
        {
          ...
          repondre(`  Exemple :* ${prefixe}boom ʙᴜsᴛᴀʀᴢᴏɴᴇ ᴍᴅ`)
          return ;
        }
       try{
              var radio2 = "e0723d60-fc0d-421f-bf8f-a9b9b61e4be6";
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by ʙᴜsᴛᴀʀᴢᴏɴᴇ ᴍᴅ*"})
...
var {ms,repondre,arg,prefixe}=commandeOptions;
  if(!arg||arg=="")
  {
    repondre(`${prefixe}water ʙᴜsᴛᴀʀᴢᴏɴᴇ ᴍᴅ`)
    return;
  }
  try{
...
zokou({ nomCom: "snow", categorie: "Logo", reaction: "❄️" }, async (dest, zk, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/create-beautiful-3d-snow-text-effect-online-1101.html", text)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: 'Logo by ʙᴜsᴛᴀʀᴢᴏɴᴇ ᴍᴅ' }, { quoted: ms });
    })
    .catch((err) => {
...
zokou({ nomCom: "thunder", categorie: "Logo", reaction: "⚡" }, async (dest, zk, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/online-thunder-text-effect-generator-1031.html", text)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: 'Logo by bustarzone md' }, { quoted: ms });
    })
    .catch((err) => {
      console.error("Une erreur s'est produite :", err);
    });
